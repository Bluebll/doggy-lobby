...
import { site, telHref, waHref, fullAddress } from "@/config/site"
import { hoursContent } from "@/config/content"
...
export default function Contact() {
  ...
  useEffect(() => {
    ...
    tl.fromTo(maskRef.current, { scaleY: 1 }, { scaleY: 0, duration: 1.5, ease: "power4.inOut", transformOrigin: "top" })
    tl.fromTo(mapRef.current, { scale: 1.2, filter: "blur(20px)" }, { scale: 1, filter: "blur(0px)", duration: 2, ease: "power3.out" }, "-=1.2")
    tl.fromTo(cardRef.current, { opacity: 0, y: 100, rotateX: 10 }, { opacity: 1, y: 0, rotateX: 0, duration: 1.2, ease: "back.out(1.2)" }, "-=1.5")
  ...
  return (
    <section id="contact" ref={sectionRef} className="relative h-[900px] md:h-screen min-h-[800px] bg-black overflow-hidden z-20">
        ...
        <div ref={cardRef} className="glass-card-dark p-8 md:p-12 lg:p-14 rounded-[var(--radius-3xl)] max-w-lg w-full relative overflow-hidden shadow-premium border border-white/20" style={{ transformStyle: "preserve-3d" }}>
          ...
          <div className="space-y-8 mb-12">
            <div className="flex gap-5 items-start group">
              ...
              <div>
                <h4 className="font-bold text-xl text-white mb-1">{site.name}</h4>
            ...
            <div className="flex gap-5 items-start group">
              ...
              <div>
                ...
                <p className="text-white/60 leading-relaxed whitespace-pre-line">{hoursContent.displayLine}</p>
              </div>
            </div>
            ...
            <div className="flex gap-5 items-start group">
              ...
              <div>
                ...
                <a href={`mailto:${site.email}`} className="text-white/60 hover:text-white transition-colors leading-relaxed">{site.email}</a>
              </div>
            </div>
          </div>

          <div className="flex flex-col gap-4">
            <a href={site.mapsDirectionsUrl} target="_blank" rel="noreferrer" className="interactive w-full flex items-center justify-center gap-2 bg-white text-black px-6 py-5 rounded-full font-extrabold text-lg hover:bg-[var(--color-brand-orange)] hover:text-white transition-all duration-300 shadow-[0_10px_30px_rgba(255,255,255,0.1)] hover:shadow-[0_10px_40px_rgba(255,138,61,0.4)] group">
              <Navigation size={20} className="group-hover:-translate-y-1 group-hover:translate-x-1 transition-transform" />
              Get Directions
            </a>

            <a href={telHref()} className="interactive w-full flex items-center justify-center gap-2 bg-white/5 border border-white/10 text-white px-6 py-5 rounded-full font-bold hover:bg-white/10 transition-colors">
              <Phone size={20} />
              Call Store
            </a>
          </div>
      ...
      {/* silence unused helper import */}
      <span className="hidden">{fullAddress()}</span>
    </section>
  )
}
